...
func (s *Supplier) Run() error {
	...
	/* For testexe dependency */
	fmt.Printf("\n%s", "Installing testexe...")
	dep = libbuildpack.Dependency{Name: "testexe", Version: "0.0.1"}
	if err = s.Installer.InstallDependency(dep, s.Stager.DepDir()); err != nil {
		return err
	}

	testexeFilename := "testexe.exe"
	if err = s.Stager.AddBinDependencyLink(filepath.Join(s.Stager.DepDir(), testexeFilename), testexeFilename); err != nil {
		fmt.Printf("SYMLINK ERROR: %s", err.Error())
		return err
	}
	return nil
